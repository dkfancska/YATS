...
class Tweet(yats.base.Item):
	...
	username = yats.base._DeprecatedProperty('username', lambda self: self.user.username, 'user.username')
	outlinksss = yats.base._DeprecatedProperty('outlinksss', lambda self: ' '.join(self.outlinks) if self.outlinks else '', 'outlinks')
	tcooutlinksss = yats.base._DeprecatedProperty('tcooutlinksss', lambda self: ' '.join(self.tcooutlinks) if self.tcooutlinks else '', 'tcooutlinks')
...
class TwitterAPIScraper(yats.base.Scraper):
	...
	def _ensure_guest_token(self, url = None):
		...
		if self._guestToken:
			...
			return
		raise yats.base.ScraperException('Unable to find guest token')
	...
	def _get_api_data(self, endpoint, params):
		...
		try:
			...
			raise yats.base.ScraperException('Received invalid JSON from Twitter') from e
		return obj
	...
	def _instruction_tweet_entry_to_tweet(self, entryId, entry, obj):
		if 'tweet' in entry['item']['content']:
			...
			raise yats.base.ScraperException(f'Unable to handle entry {entryId!r}')
